...
const LeisurePage = () => {
  ...
  const { leisure, loading, error } = useLeisure(page, pageSize);
  ...
  return (
    <div className="container mt-4">
      <h2 className="mb-4 text-center">Leisure Spots</h2>
      ...
      {/* 데이터 표시 */}
      {!loading && leisure.length === 0 && <p className="text-center">No leisure spots found.</p>}
      ...
      <div className="d-flex justify-content-center mt-4">
        {Array.from({ length: Math.ceil(leisure.length / pageSize) }, (_, i) => i + 1).map((pageNumber) => (
          <button
            key={pageNumber}
            className={`btn mx-1 ${
